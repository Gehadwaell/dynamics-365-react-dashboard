...
export const usePurchaseRequisitions = () => {
  ...
  const createRequisition = async (requisitionData) => {
    ...
    try {
      // 🔥 FIX: Ensure we are sending a string, not an object
      const actualName =
        typeof requisitionData === "object"
          ? requisitionData.RequisitionName ||
            requisitionData.name ||
            Object.values(requisitionData)[0] ||
            "New Requisition"
          : requisitionData;

      const payload = {
        RequisitionName: String(actualName),
        BuyingLegalEntityId: "usmf", // D365 usually strictly requires this!
      };
      ...
      if (res.ok) {
        ...
        toast.error(
          `Error: ${errorData?.error?.message || "Failed to create header"}`,
          { id: loadingToast },
        );
        return null;
      }
  ...
  const createRequisitionLine = async (reqId, lineData) => {
    ...
    try {
      const reqHeader =
        allRequisitions.find((r) => r.RequisitionNumber === reqId) || {};
      ...
      const payload = {
        ...
        RequisitionLineNumber:
          reqLines.length > 0
            ? Math.max(...reqLines.map((l) => l.RequisitionLineNumber)) + 1
            : 1,
      ...
      if (lineData.warehouseId)
        payload.ReceivingWarehouseId = lineData.warehouseId;
      ...
      if (lineData.price) {
        ...
        payload.LineAmount =
          parseFloat(lineData.price) * parseFloat(lineData.quantity);
      }
  ...
  const deleteRequisitionLine = async (reqId, lineNumber) => {
    ...
    try {
      ...
      if (res.ok) {
        setReqLines((prev) =>
          prev.filter((line) => line.RequisitionLineNumber !== lineNumber),
        );
      ...
      const errorData = await res.json();
      const FAndOError =
        errorData?.error?.innererror?.message ||
        errorData?.error?.message ||
        "Unknown D365 Error";

      toast.error(`D365 Blocked Deletion: ${FAndOError}`, {
        id: loadingToast,
        duration: 6000,
      });
      return false;
    ...
    const load = async () => {
      try {
        ...
        const [prRes, sRes, wRes, cRes] = await Promise.all([
          fetch(
            "/api-data/data/PurchaseRequisitionHeaders?$orderby=RequisitionNumber desc&$top=100",
            { headers },
          ),
  ...
  const filtered = allRequisitions.filter(
    (r) =>
      (r.RequisitionNumber || "")
        .toLowerCase()
