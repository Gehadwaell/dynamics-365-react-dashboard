...
import { Settings, HelpCircle, ShoppingCart, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      ...
      <button 
        ...
        className="lg:hidden fixed top-5 left-5 z-[70] p-2.5 bg-white border border-gray-100 rounded-xl shadow-lg text-blue-800 transition-transform active:scale-90"
      >
        {isOpen ? <X size={20} /> : <Menu size={20} />}
      </button>

      {/* 2. MOBILE BACKGROUND BLUR */}
      {isOpen && (
        <div 
      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto pb-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          ...
          <nav className="space-y-2">
            ...
            <button 
              onClick={handleSalesClick}
              className={`group w-full flex items-center justify-start gap-3 px-4 py-2.5 rounded-2xl font-bold transition-all duration-300 border cursor-pointer active:scale-95 overflow-hidden relative ${
                isHomeActive 
            ...
            >
              ...
              <span className="tracking-tight relative z-10 text-sm">Sales Orders</span>
            </button>
          </nav>
        </div>
